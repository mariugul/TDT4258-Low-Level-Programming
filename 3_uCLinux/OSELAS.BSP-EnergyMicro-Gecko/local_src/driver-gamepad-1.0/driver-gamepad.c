...
static int my_fasync(int fd,  struct file *filp, int mode){
    printk("mu_fansync called1!\n");
    fasync_helper(fd, filp,mode,&asqueue);
    printk("mu_fansync called2!\n");
    return 0;
}
...
static irqreturn_t int_handler(int irq, void *dev_id){
    printk("int_handler executed (driver), irq: %d\n", irq);
    kill_fasync(&asqueue,SIGIO, POLL_IN);
    return IRQ_HANDLED;
}


void initialize_interrupts(void){
    request_irq(17,&int_handler,IRQF_SHARED,"gamePad",&devno);
    request_irq(18,&int_handler,IRQF_SHARED,"gamePad",&devno);
}
...
static int __init my_init(void)
{
    ...
    ret_cdev_add = cdev_add(&my_cdev, devno, 1); //not sure about the 0, is the first device number to wich this device responds
